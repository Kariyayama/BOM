...
```{r}
# 細胞クラスターの一覧を取得する                                             
read_marker_gene_list("Hs", input, "_") -> Hs_all
read_marker_gene_list("Mm", input, "_") -> Mm_all
...
GO解析
```{r}
run_GOanalysis_for_all_ident("Hs", input, hsapiens, cluster.list)
run_GOanalysis_for_all_ident("Mm", input, mmuculus, cluster.list)
```
GO解析の結果をプロットして可視化
GO解析の結果を読み込み
```{r}
