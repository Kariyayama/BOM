...
hs_mm_marker_disease <- read_tsv("MGI_DO/hs_mm_marker_disease_human_gene_version2.tsv",
                                 show_col_types = FALSE)
# ヒトとマウスで異なる遺伝子が紐づけられたDO
hs_mm_marker_disease.different_bg <- read_tsv("MGI_DO/hs_mm_marker_disease_human_gene_different_bg_version2.tsv",
                                              show_col_types = FALSE)
# ヒトのみ遺伝子が紐づけられているDO
hs_only_marker_disease <- read_tsv("MGI_DO/hs_only_marker_disease_human_gene_version2.tsv",
                                   show_col_types = FALSE)
# ヒトの遺伝子が紐づけられているDO
hs_marker_disease <- read_tsv("MGI_DO/hs_marker_disease_human_gene_version2.tsv",
                              show_col_types = FALSE)
...
bind_rows(Hs_Mm.DO_marker, Hs_Mm.DO_marker.different_bg,
    ...
    ggplot(aes(x=reorder(cell_type_label, n), y = n, fill = share)) +
    geom_bar(stat= "identity") +
    facet_grid(type ~ organ, scale = "free", space = "free_x") +
    theme_bw() +
    theme(axis.text.x =
              element_text(angle = 90, hjust = 1, vjust = 0.5))

# その細胞種の全CCS遺伝子のHs_onlyとshareの数で正規化した
bind_rows(Hs_Mm.DO_marker, Hs_Mm.DO_marker.different_bg,
          Hs_only.DO_marker, Hs.DO_marker) %>%
    mutate(organ = str_split(cell_type_label, "_", simplify = TRUE)[1]) %>%
    inner_join(cell_type_ontology, by = "cell_type_label") %>%
    inner_join(hs_marker_count, by = c("share", "cell_label")) %>%
    mutate(normalized_count = n.x/n.y * 500) %>%
    ggplot(aes(x=reorder(cell_type_label, n), y = n, fill = share)) +
    geom_bar(stat= "identity") +
    facet_grid(type ~ organ, scale = "free", space = "free_x") +
    theme_bw() +
...
# その細胞種の全CCS遺伝子のHs_onlyとshareの数で正規化して、比をプロット
bind_rows(Hs_Mm.DO_marker, Hs_Mm.DO_marker.different_bg,
          Hs_only.DO_marker, Hs.DO_marker) %>%
    mutate(organ = str_split(cell_type_label, "_", simplify = TRUE)[1]) %>%
    inner_join(cell_type_ontology, by = c("cell_type_label", "organ")) %>%
    inner_join(hs_marker_count, by = c("share", "cell_label")) %>%
    mutate(normalized_count = n.x/n.y * 500) %>%
    ggplot(aes(x=reorder(cell_type_label, normalized_count),
               y = normalized_count, fill = share)) +
